...
import EditModalAppointment from './EditModalAppointment'
import DeleteModalAppointment from './DeleteModalAppointment'
...
function CardAppointment({ appointment }) {
	...
	const handleEditSave = (date, hour) => {
		console.log(`Editar cita con el dia ${date} y la hora ${hour}`)
	}

	// funcion para eliminar la cita
	const handleDeleteConfirm = (id) => {
		console.log(`Eliminar cita con el id ${id}`)
	}
		...
		<div className="bg-slate-900 border-2 border-slate-700 rounded-xl p-4 sm:p-5 hover:border-amber-400 transition-all duration-200 hover:shadow-lg">
			...
			{editAppointment && (
				<EditModalAppointment
					...
					fechaActual={
						(typeof editAppointment.date_time === 'string'
							? editAppointment.date_time
							: editAppointment.date_time.toISOString()
						).split('T')[0]
					}
					horaActual={(typeof editAppointment.date_time === 'string'
						? editAppointment.date_time
						: editAppointment.date_time.toISOString()
					)
						.split('T')[1]
			...
			{deleteAppointment && (
				<DeleteModalAppointment
					...
					onConfirm={() => {
						const id =
							typeof deleteAppointment.id === 'string'
					...
					citaId={
						typeof deleteAppointment.id === 'string'
							? deleteAppointment._d
							: undefined
					}
					clienteNombre={deleteAppointment.client.name || 'el cliente'}
				/>
			)}
		</div>
	)
